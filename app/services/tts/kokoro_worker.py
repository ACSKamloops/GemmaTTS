"""
Kokoro TTS Worker — Kokoro 82M via kokoro-onnx
https://github.com/thewh1teagle/kokoro-onnx

Uses kokoro-onnx v0.5.0 with ONNX model and voice embeddings.
Output: 24 kHz WAV.

Voice files are individual .pt embeddings in models/kokoro/voices/.
The create() method accepts either a voice name string (if using a
consolidated voices.bin) or a numpy embedding array (our approach).
"""
import os
import io
import logging
...
logger = logging.getLogger("kokoro-worker")
...
class KokoroWorker:
    ...
    def load(self):
        ...
        # kokoro-onnx constructor requires a voices file for np.load().
        # We have individual .pt voice files, so we create a minimal
        # voices .npy that satisfies the constructor, then pass voice
        # embeddings directly to create() as numpy arrays.
        ...
        # Use CUDA if available via environment variable
    ...
    def _load_voice_embedding(self, voice_id: str) -> np.ndarray:
        """Load a voice embedding from .pt or .bin file."""
    ...
    def _resolve_voice(self, voice_id: str) -> np.ndarray:
        """
        Resolve a voice ID to a numpy embedding.
        Supports blending via '+' separator (e.g. 'af_heart+am_adam').
        """
        if "+" in voice_id:
    ...
    def synthesize(self, text: str, voice_id: str = "default") -> tuple[bytes, int]:
        ...
        Args:
            text: Text to speak.
            voice_id: Voice name (e.g. 'af_heart', 'am_adam', 'bf_emma').
                      Use '+' for blending: 'af_heart+am_adam'.
